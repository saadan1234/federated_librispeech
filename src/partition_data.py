...
log_dir = Path(
    '/home/saadan/scratch/federated_librispeech/src/logs/partition_logs')
...
# Optimize the number of workers based on CPU cores and memory allocated on the node
...
def get_optimal_worker_count() -> int:
    ...
    optimal_workers = min(memory_limited_workers,
                          cpu_limited_workers, logical_cores)

    logger.info(
        f"System resources: {cpu_cores} physical cores, {logical_cores} logical cores, {memory_gb:.1f}GB memory")
    logger.info(f"Optimal worker count: {optimal_workers}")

    return optimal_workers

# TODO: This is a placeholder for the actual audio file processing
...
# TODO: This is a placeholder for the actual chapter directory processing
...
def process_chapter_directory(chapter_args: Tuple[Path, str, str]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(
            f"Error processing chapter {speaker_id}-{chapter_id}: {e}")
        return None

# TODO: This is a placeholder for the actual speaker directory processing
...
def process_speaker_directory(speaker_args: Tuple[Path, str]) -> Tuple[str, Dict[str, Any]]:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            chapter_results = list(executor.map(
                process_chapter_directory, chapter_dirs))
...
def copy_file_safe(src_dest_pair: Tuple[Path, Path], max_retries: int = 3, verify_copy: bool = True) -> bool:
    ...
    try:
        ...
        dest_free_space = shutil.disk_usage(dest.parent).free if dest.parent.exists(
        ) else shutil.disk_usage(dest.parent.parent).free

        if src_size > dest_free_space:
            logger.error(
                f"Insufficient disk space to copy {src} (need {src_size} bytes, have {dest_free_space} bytes)")
            return False
    except Exception as e:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if verify_copy:
                if not dest.exists():
                    raise FileNotFoundError(
                        f"Destination file was not created: {dest}")

                dest_size = dest.stat().st_size
                if src_size != dest_size:
                    raise ValueError(
                        f"File size mismatch: src={src_size}, dest={dest_size}")

            return True

        except PermissionError as e:
            logger.warning(
                f"Permission error copying {src} to {dest} (attempt {attempt + 1}/{max_retries}): {e}")
            ...
            if e.errno == 28:  # No space left on device
                ...
                logger.error(
                    f"Filename too long - cannot copy {src} to {dest}: {e}")
                return False
            else:
                logger.warning(
                    f"OS error copying {src} to {dest} (attempt {attempt + 1}/{max_retries}): {e}")
                if attempt < max_retries - 1:
                    time.sleep(0.1 * (attempt + 1))

        except Exception as e:
            logger.warning(
                f"Unexpected error copying {src} to {dest} (attempt {attempt + 1}/{max_retries}): {e}")
            if attempt < max_retries - 1:
                time.sleep(0.1 * (attempt + 1))
...
class FlowerLibriSpeechPartitioner:
    ...
    def _setup_directories(self):
        ...
        logger.info(
            f"Created directory structure for {self.num_clients} clients")
    ...
    def analyze_dataset_structure(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Analyzing dataset structure for subsets: {subsets} using {self.max_workers} workers")
                ...
                with tqdm(total=len(speaker_dirs), desc=f"Processing speakers in {subset}") as pbar:
                    for future in as_completed(future_to_speaker):
                        ...
                        try:
                            ...
                            logger.error(
                                f"Error processing speaker {speaker_id}: {e}")
                            pbar.update(1)
        ...
        total_files = sum(data['total_files']
                          for data in speakers_data.values())
        total_hours = sum(data['total_duration']
                          for data in speakers_data.values()) / 3600
    ...
    def create_speaker_partitions(self, speakers_data: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        for speaker_stat in speaker_stats:
            ...
            min_client = min(client_loads.keys(),
                             key=lambda x: client_loads[x])
        ...
        for client_id, client_speakers in client_partitions.items():
            ...
            val_speakers_count = max(
                1, int(len(client_speakers) * self.validation_split))
        ...
        if 'global_test_path' in self.data_config:
            ...
            if global_test_path.startswith('/'):
                # Absolute path
                global_test_speakers = self.load_global_dataset_speakers(
                    global_test_path)
            else:
        ...
        if 'global_validation_path' in self.data_config:
            ...
            if global_val_path.startswith('/'):
                # Absolute path
                global_val_speakers = self.load_global_dataset_speakers(
                    global_val_path)
            else:
                # Relative to librispeech_source
        ...
        if not global_test_speakers or not global_val_speakers:
            logger.warning(
                "Global datasets not found, falling back to train-clean-100 splits")
            all_speakers = list(speakers_data.keys())
            random.shuffle(all_speakers)

            global_test_count = max(
                1, int(len(all_speakers) * self.test_split))
            global_val_count = max(
                1, int(len(all_speakers) * self.validation_split))

            global_test_speakers = all_speakers[:global_test_count]
            global_val_speakers = all_speakers[global_test_count:
                                               global_test_count + global_val_count]
        ...
        for client_id, data in partitions.items():
            if client_id != 'global':
                ...
                logger.info(
                    f"  {client_id}: {train_count} train, {val_count} val speakers")
    ...
    def copy_partition_data_parallel(self, speaker_id: str, speakers_data: Dict[str, Any],
        ...
        for chapter_info in speaker_data['chapters']:
            ...
            if chapter_info['transcript_file']:
                ...
                transcript_dest = dest_transcript_dir / \
        ...
        if all_operations:
            logger.debug(
                f"Processing {len(all_operations)} file operations for speaker {speaker_id}")
            ...
            with ThreadPoolExecutor(max_workers=min(16, len(all_operations))) as executor:
                ...
                for i, (src, dest) in enumerate(all_operations):
                    ...
                    verify = getattr(self, 'verify_copies', False) or str(
                        dest).endswith('.txt')
                    max_retries = getattr(self, 'max_retries', 3)
                    enhanced_operations.append(
                        (src, dest, max_retries, verify))
                ...
                copy_results = list(executor.map(
                    enhanced_copy_task, enhanced_operations))

            failed_copies = sum(1 for result in copy_results if not result)
            success_rate = (len(copy_results) - failed_copies) / \
                len(copy_results) * 100 if copy_results else 0
            ...
            if failed_copies > 0:
                ...
                logger.debug(
                    f"Successfully copied all {len(all_operations)} files for speaker {speaker_id}")
        ...
        for chapter_info in speaker_data['chapters']:
            ...
            if chapter_info['transcript_file']:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to copy transcript {transcript_src}: {e}")
    ...
    def copy_global_dataset_speaker(self, speaker_id: str, dataset_type: str,
        ...
        if not speaker_dir.exists():
            logger.warning(
                f"Speaker {speaker_id} not found in {global_dataset_path}")
            return []
        ...
        for chapter_dir in speaker_dir.iterdir():
            ...
            if trans_file.exists():
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to copy transcript {trans_file}: {e}")
    ...
    def create_client_data(self, client_id: str, partition_data: Dict[str, List[str]],
        ...
        if train_speakers:
            logger.info(
                f"Processing {len(train_speakers)} training speakers for {client_id}")
        ...
        if val_speakers:
            logger.info(
                f"Processing {len(val_speakers)} validation speakers for {client_id}")
        ...
        if all_manifest_entries:
            ...
            logger.info(
                f"Saved {len(all_manifest_entries)} entries to {manifest_file}")
    ...
    def create_global_datasets(self, partitions: Dict[str, Any], speakers_data: Dict[str, Any]):
        ...
        logger.info(
            f"Processing {len(val_speakers)} validation speakers for global dataset")
        ...
        if internal_val_tasks:
            with ThreadPoolExecutor(max_workers=min(self.max_workers, len(internal_val_tasks))) as executor:
                ...
                internal_results = list(tqdm(
                    executor.map(process_internal_val_task,
                                 internal_val_tasks),
        ...
        if external_val_tasks:
            with ThreadPoolExecutor(max_workers=min(self.max_workers, len(external_val_tasks))) as executor:
                ...
                external_results = list(tqdm(
                    executor.map(process_external_val_task,
                                 external_val_tasks),
        ...
        logger.info(
            f"Processing {len(test_speakers)} test speakers for global dataset")
        ...
        if internal_test_tasks:
            with ThreadPoolExecutor(max_workers=min(self.max_workers, len(internal_test_tasks))) as executor:
                ...
                internal_results = list(tqdm(
                    executor.map(process_internal_test_task,
                                 internal_test_tasks),
        ...
        if external_test_tasks:
            with ThreadPoolExecutor(max_workers=min(self.max_workers, len(external_test_tasks))) as executor:
                ...
                external_results = list(tqdm(
                    executor.map(process_external_test_task,
                                 external_test_tasks),
    ...
    def _validate_system_requirements(self) -> bool:
        ...
        try:
            ...
            estimated_space_gb = sum(subset_sizes.get(subset, 25)
                                     for subset in subsets)
            ...
            if free_space_gb < estimated_space_gb:
                ...
                logger.info(
                    f"Disk space check passed: {free_space_gb:.1f}GB available")
        ...
        for subset in (self.dataset_subset if isinstance(self.dataset_subset, list) else [self.dataset_subset]):
            ...
            if not dataset_path.exists():
                ...
                if not tar_path.exists():
                    logger.error(
                        f"Neither dataset directory {dataset_path} nor tar file {tar_path} exists")
                    return False
        ...
        try:
            ...
            logger.error(
                f"No write permission to output directory {self.output_root}: {e}")
            return False
    ...
    def run_partitioning(self, global_only: bool = False) -> bool:
        ...
        if not getattr(self, 'skip_validation', False):
            if not self._validate_system_requirements():
                logger.error(
                    "System requirements validation failed. Aborting partitioning.")
                return False
        else:
            logger.warning(
                "System requirements validation SKIPPED (--skip-validation flag used)")

        start_time = time.time()
        logger.info(
            "Starting LibriSpeech partitioning for Flower simulation...")
        ...
        try:
            ...
            if not self.extract_dataset():
                return False
            logger.info(
                f"✓ Dataset extraction completed in {time.time() - step_start:.2f}s")

            # Step 2: Analyze dataset structure
            step_start = time.time()
            dataset_analysis = self.analyze_dataset_structure()
            logger.info(
                f"✓ Dataset analysis completed in {time.time() - step_start:.2f}s")

            # Step 3: Create speaker partitions
            step_start = time.time()
            partitions = self.create_speaker_partitions(
                dataset_analysis['speakers_data'])
            logger.info(
                f"✓ Speaker partitioning completed in {time.time() - step_start:.2f}s")
            ...
            for i in range(self.num_clients):
                ...
                stats = self.create_client_data(
                    client_id, partition_data, dataset_analysis['speakers_data'])
                client_stats.append(stats)
            logger.info(
                f"✓ Client data creation completed in {time.time() - step_start:.2f}s")

            # Step 5: Create global datasets
            step_start = time.time()
            self.create_global_datasets(
                partitions, dataset_analysis['speakers_data'])
            logger.info(
                f"✓ Global datasets creation completed in {time.time() - step_start:.2f}s")

            # Step 6: Save metadata
            step_start = time.time()
            metadata = self.save_partition_metadata(partitions, client_stats)
            logger.info(
                f"✓ Metadata saving completed in {time.time() - step_start:.2f}s")

            # Step 7: Validate partition
            step_start = time.time()
            validation_passed = self.validate_partition()
            logger.info(
                f"✓ Partition validation completed in {time.time() - step_start:.2f}s")

            if validation_passed:
                logger.info("🎉 Partitioning completed successfully!")
                logger.info(f"📊 Summary:")
                logger.info(
                    f"  - {len(dataset_analysis['speakers_data'])} total speakers")
                logger.info(
                    f"  - {metadata['global_statistics']['total_train_files']} total training files")
                logger.info(
                    f"  - {metadata['global_statistics']['total_train_hours']:.2f} total training hours")
                logger.info(
                    f"  - Average {metadata['global_statistics']['avg_files_per_client']:.1f} files per client")
                total_time = time.time() - start_time
                logger.info(
                    f"🕒 Total processing time: {total_time:.2f}s ({total_time/60:.1f} minutes)")
                return True
            else:
    ...
    def run_global_dataset_creation(self) -> bool:
        ...
        try:
            ...
            if not metadata_file.exists():
                logger.error(
                    f"No existing partition metadata found at {metadata_file}")
                logger.error(
                    "Run full partitioning first or provide partition metadata")
                return False

            logger.info(
                f"Loading existing partition metadata from {metadata_file}")
...
def main():
    ...
    if not args.force and not args.global_only:
        existing_metadata = partitioner.output_root / \
            "global" / "partition_metadata.json"
        if existing_metadata.exists():
            logger.info(
                f"Partition already exists at {partitioner.output_root}")
            logger.info(
                "Use --force to re-partition or --global-only to recreate global datasets")
            return
    ...
    if success:
        logger.info("✅ Partitioning completed successfully!")
        logger.info(
            f"📁 Partitioned data available at: {partitioner.output_root}")
